...
namespace CL {
    ...
    public:
        ParsingException(Token error_token, std::string_view error_message)
                : m_error_token(std::move(std::move(error_token))), CLException(std::string(error_message)) {
        }
    };
    ...
    ExprPtr Parser::module_expression() {
        consume("Module definitions must start with a {", TokenType::Left_Curly_Brace);
        ExprList list;
    ...
    ExprPtr Parser::for_expression() {
        auto name = consume("For expressions start with an identifier", TokenType::Identifier).get<String>();
        consume("For expressions must have an \"in\" after the identifier", TokenType::In);
    ...
    Names Parser::arg_names() {
        ...
        if (!match(TokenType::Right_Brace)) {
            do {
                auto tok = consume("Arguments can only be identifiers", TokenType::Identifier);
        ...
        if (match(TokenType::Dot, TokenType::Left_Square_Brace)) {
            while (match(TokenType::Dot, TokenType::Left_Square_Brace)) {
                ...
                if (previous().get_type() == TokenType::Left_Square_Brace) {
                    what = expression();
                    consume("Indexing expressions end with a ]", TokenType::Right_Square_Brace);
                } else {
                    auto next = consume("Named indexing expressions expect an identifier",
    ...
    ExprPtr Parser::literal() {
        ...
        if (match(TokenType::Number)) {
            ...
            consume("Grouping expressions must end with a )", TokenType::Right_Brace);
            return expr;
    ...
    ExprPtr Parser::dict_expression() {
        consume("dict expressions begin with a { after the dict keyword", TokenType::Left_Curly_Brace);
    ...
    ExprPtr Parser::list_expression() {
        consume("list expressions begin with a [ after the list keyword", TokenType::Left_Square_Brace);
    ...
    bool Parser::match_expression_begin() {
        ...
        return (p == TokenType::Identifier || p == TokenType::Number || p == TokenType::Plus || p == TokenType::Minus ||
                p == TokenType::Not || p == TokenType::Left_Brace || p == TokenType::Self);
    }
} // namespace CL
